...
class DHClient(Client):
    ...
    def negotiate(self):
        ...
        # Alice generates a private number a -> the vulnerability here is that a is 0
        a = 0
        ...
        R = pow(B, a, p)
        logger.debug(f"Computed secret key: {R}")

        key = hashlib.sha256(long_to_bytes(R)).digest()
        ...
        if data.get("success"):
            decoded_message = unpad(R.decrypt(b64decode(data["message"])), 16)
